                                   ...
                                   DWORD dwPrimaryBitRate )
{
    ...
    if( FAILED( hr = DirectSoundCreate( NULL, &m_pDS, NULL ) ) )
        return DXTRACE_ERR( TEXT("DirectSoundCreate9"), hr );
...
//-----------------------------------------------------------------------------
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags,float volume )
{
    HRESULT hr;
    BOOL    bRestored;
	...
	pDSB->SetVolume((long)((1-volume)*(DSBVOLUME_MIN -DSBVOLUME_MAX)));
	pDSB->Play( 0, dwPriority, dwFlags );
